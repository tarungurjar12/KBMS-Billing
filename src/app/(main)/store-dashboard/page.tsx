...
export default function StoreManagerDashboardPage() {
  ...
  const fetchManagerDashboardData = useCallback(async () => {
    ...
    setRecentActivity([]); // Clear previous activity
    ...
    try {
      ...
      if (error.code === 'failed-precondition') {
        toast({
            ...
            description: `One or more queries for your dashboard data failed. Please create the required Firestore index. Check your browser's developer console for a Firebase error message that includes a link to create the required index. You can also create indexes manually in your Firebase console.`,
            variant: "destructive",
            duration: 20000,
        });
      } else {
      ...
      setRecentActivity(["Failed to load recent activity due to an error."]);
        ...
        // Ensure all individual metric loading states are false
        setMetrics(prevMetrics => prevMetrics.map(m => ({...m, isLoading: false})));
    }
  }, [currentUser, toast]); // Removed 'metrics' from dependencies
